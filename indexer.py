...
for filename in  :
    ...
    for word in allFiles[filename]:
        tfIdf[filename][word] = allFiles[filename][word] * math.log10(allFiles.__len__()/indexTable[word])

for item in tfIdf:
    print("in a file name " + str(item) + " --> " + str(tfIdf[item]))
for item in indexTable:
    print("\"" + str(item) + "\"" + " wtih the weight of: " + str(indexTable[item]))
...
for item in queryVector:
    if item in indexTable:
        queryVector[item] = queryVector[item] * math.log10(allFiles.__len__()+1/indexTable[item]+1)
    else:
        queryVector[item] = queryVector[item] * math.log10(allFiles.__len__() + 1)
